...
(function initConstellation(): void {
    ...
    // Define particle type with position, size, and movement properties
    interface Particle {
        x: number;
        y: number;
        r: number;
    ...
    function drawMouseConnections(targetX: number, targetY: number): void {
        ...
        nearby.forEach(({ p, dist }) => {
            ...
            if (dist < maxDist) {
                const alpha = Math.round(
                    (1 - dist / (CONNECT_DISTANCE * MOUSE_CONNECTION_DISTANCE_MULTIPLIER)) *
